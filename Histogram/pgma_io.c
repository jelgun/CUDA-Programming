...
/*
  ...
  Parameters:
    ...
    Input, int XSIZE, YSIZE, the number of columns and rows of data.
...
void pgma_write_old ( char *file_out_name, int xsize, int ysize, int *g )
...
*/
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "PGMA_WRITE_OLD - Fatal error!\n" );
    fprintf ( stderr, "  Cannot open the output file \"%s\".\n", file_out_name );
    exit ( 1 );
  }
/*
...
/*
  Purpose:
    ...
    TEST_PGMA_READ tests the ASCII PGM read routines.
...
/*
  Purpose:
    ...
    TEST_PGMA_WRITE tests the ASCII PGM write routines.
...
*/
{
  char comment[] = "Test file created by test_pgma_write";
  int *gray;
  int maxval;
  int xsize;
  int ysize;
...
/*
  Allocate memory.
*/
  gray = ( int * ) malloc ( xsize * ysize * sizeof ( int ) );
/*
  Set the data.
*/
  pgma_example ( xsize, ysize, gray );
/*
  Set the maximum allowed value to the maximum value.
*/
  maxval = i4mat_max ( xsize, ysize, gray );
/*
  Write the data to the file.
*/
  pgma_write ( file_name, comment, xsize, ysize, maxval, gray );
/*
  Free memory.
*/
  free ( gray );
